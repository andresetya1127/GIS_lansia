...
<div>
    <div x-data="{ currentlyReorderingStatus: false }">
        <div {{ $this->getTopLevelAttributes() }}>
            ...
            <x-livewire-tables::wrapper :tableName="$tableName" :$primaryKey :$isTailwind :$isBootstrap :$isBootstrap4
                :$isBootstrap5 :$localisationPath>
                ...
                <x-livewire-tables::table>
                    ...
                    <x-slot name="thead">
                        ...
                        @tableloop($this->selectedVisibleColumns as $index => $column)
                            <x-livewire-tables::table.th wire:key="{{ $tableName . '-table-head-' . $index }}" :$column
                                :$index />
                        @endtableloop
                    </x-slot>
                    ...
                    @if (count($currentRows = $this->getRows) > 0)
                        ...
                        @tableloop($currentRows as $rowIndex => $row)
                            <x-livewire-tables::table.tr wire:key="{{ $tableName }}-row-wrap-{{ $row->{$primaryKey} }}"
                                :$row :$rowIndex>
                                @if ($getCurrentlyReorderingStatus)
                                    <x-livewire-tables::table.td.reorder x-cloak x-show="currentlyReorderingStatus"
                                        ...
                                        :rowID="$tableName . '-' . $row->{$this->getPrimaryKey()}" :$rowIndex />
                                @endif
                                @if ($showBulkActionsSections)
                                    <x-livewire-tables::table.td.bulk-actions
                                        wire:key="{{ $tableName }}-row-bulk-act-{{ $row->{$primaryKey} }}" :$row
                                        :$rowIndex />
                                @endif
                                @if ($showCollapsingColumnSections)
                                    <x-livewire-tables::table.td.collapsed-columns
                                        wire:key="{{ $tableName }}-row-collapsed-{{ $row->{$primaryKey} }}" :$rowIndex />
                                @endif
